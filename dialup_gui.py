...
class DialupGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        title = ttk.Label(root, text="Retro Dial-Up Transfer", font=("Courier", 20, "bold"))
        title.pack(anchor="w", pady=(0, 6))
        subtitle = ttk.Label(root, text="Windows / macOS LAN file transfer with nostalgic slow mode")
        ...
        self.log_text = tk.Text(log_wrap, height=16, wrap="word", font=("Courier", 11))
    ...
    def _build_receive_tab(self, parent: ttk.Frame) -> None:
        ...
        self._row(parent, 0, "Bind Host", self.recv_host)
        self._row(parent, 1, "Port", self.recv_port)
        self._row(parent, 2, "Speed (kbps)", self.recv_speed)
        self._row(parent, 3, "Output Dir", self.recv_outdir)

        chaos = ttk.Checkbutton(parent, text="Chaos mode (jitter + beeps)", variable=self.recv_chaos)
        chaos.grid(row=4, column=1, sticky="w", pady=(4, 8))
    ...
    def _build_send_tab(self, parent: ttk.Frame) -> None:
        ...
        self._row(parent, 0, "Receiver Host", self.send_host)
        self._row(parent, 1, "Port", self.send_port)
        self._row(parent, 2, "Speed (kbps)", self.send_speed)

        ttk.Label(parent, text="File").grid(row=3, column=0, sticky="w", padx=(0, 10), pady=4)
        ...
        chaos = ttk.Checkbutton(parent, text="Chaos mode (jitter + beeps)", variable=self.send_chaos)
        chaos.grid(row=4, column=1, sticky="w", pady=(4, 8))

        self.send_btn = ttk.Button(parent, text="Send File", command=self.send_once)
        self.send_btn.grid(row=5, column=1, sticky="w")

        parent.columnconfigure(1, weight=1)

    @staticmethod
    def _row(parent: ttk.Frame, row: int, label: str, var: tk.StringVar) -> None:
